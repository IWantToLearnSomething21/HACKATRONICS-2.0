...
# Warm up model Cython/C predictor
_dummy = np.zeros((1, 21), dtype=np.float64)
...
def generate_clinical_reasoning(pred_class, astv, dp, altv, ac):
    ...
    if pred_class == 1:
        if not reasoning:
            ...
            deviations = ", ".join([r.split()[1] for r in reasoning[:2]])
            return [f"Despite some minor deviations ({deviations}), the overall combination of features strongly predicts a normal physiological state."]
    elif pred_class == 2:
...
def predict_triage(metrics: PatientMetrics):
    ...
    pred_class = int(model.classes_[np.argmax(probabilities)])
