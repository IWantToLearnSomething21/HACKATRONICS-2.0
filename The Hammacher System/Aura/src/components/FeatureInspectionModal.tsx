...
export const FeatureInspectionModal: React.FC = () => {
  ...
  const [reasoning, setReasoning] = useState('All vital CTG morphological patterns are within reassuring baselines. Variability and heart rate are nominal.');
  const [action, setAction] = useState('Routine monitoring.');
  const [probs, setProbs] = useState({ normal: 0.999, suspect: 0.001, pathological: 0.0 });
  const [classCode, setClassCode] = useState<number | null>(1);
  const [isLoading, setIsLoading] = useState(false);
  const [errorMessage, setErrorMessage] = useState<string | null>(null);

  const inFlightControllerRef = React.useRef<AbortController | null>(null);

  // Direct inference call to the trained Gradient Boosting model (models/family_b_gradient_boosting.joblib)
  const analyze = async () => {
    // Abort previous in-flight request if user triggered a new assessment
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (Array.isArray(data.clinical_reasoning)) {
          setReasoning(data.clinical_reasoning.join(' '));
        } else if (typeof data.clinical_reasoning === 'string') {
      ...
      setErrorMessage(err.message || 'Failed to connect to ML backend');
    } finally {
  ...
  // One-click quick clinical presets (populates sliders, waits for user to run assessment)
  const applyPreset = (type: 'normal' | 'suspect' | 'pathological') => {
    setErrorMessage(null);
    let p: any;
    if (type === 'normal') {
      p = {
        baseline: 122, ASTV: 22, DP: 0, Mean: 113, ALTV: 0, MSTV: 4.4, AC: 0.004,
        fetalMovement: 0.018, uterineContractions: 0.003, lightDecel: 0.005, severeDecel: 0,
        MLTV: 15.2, Width: 150, Min: 50, Max: 200, Nmax: 7, Nzeros: 0, Mode: 119, Median: 117, Variance: 24, Tendency: 0
      };
    } else if (type === 'suspect') {
      p = {
        baseline: 138, ASTV: 62, DP: 0, Mean: 140, ALTV: 9, MSTV: 0.5, AC: 0,
        fetalMovement: 0.005, uterineContractions: 0.003, lightDecel: 0, severeDecel: 0,
        MLTV: 8.1, Width: 27, Min: 121, Max: 148, Nmax: 1, Nzeros: 0, Mode: 142, Median: 142, Variance: 1, Tendency: 1
      };
    } else {
      p = {
        baseline: 121, ASTV: 66, DP: 0, Mean: 85, ALTV: 0, MSTV: 2.1, AC: 0,
        fetalMovement: 0.001, uterineContractions: 0.004, lightDecel: 0.01, severeDecel: 0.001,
        MLTV: 6.4, Width: 105, Min: 55, Max: 160, Nmax: 7, Nzeros: 0, Mode: 67, Median: 92, Variance: 109, Tendency: -1
      };
    }
    ...
    setMSTV(p.MSTV);
    setAC(p.AC);
    setFetalMovement(p.fetalMovement);
    setUterineContractions(p.uterineContractions);
    setLightDecel(p.lightDecel);
    setSevereDecel(p.severeDecel);
    setMLTV(p.MLTV);
    setWidth(p.Width);
    setMin(p.Min);
    setMax(p.Max);
  ...
  const randomizeData = () => {
    ...
    if (rand < 0.5) {
      ...
      const astv = Math.floor(Math.random() * (45 - 20) + 20);
      const ac = Number((Math.random() * (0.01 - 0.002) + 0.002).toFixed(3));
      const mstv = Number((Math.random() * (2 - 1) + 1).toFixed(1));
      const mean = lb + Math.floor(Math.random() * 5);
      p = {
        baseline: lb, ASTV: astv, DP: 0, Mean: mean, ALTV: 0, MSTV: mstv, AC: ac,
        fetalMovement: Number((Math.random() * 0.05).toFixed(3)),
        uterineContractions: Number((Math.random() * (0.006 - 0.002) + 0.002).toFixed(3)),
        lightDecel: Number((Math.random() * 0.003).toFixed(3)),
        severeDecel: 0,
        MLTV: Number((Math.random() * (20 - 5) + 5).toFixed(1)),
        Width: Math.floor(Math.random() * (100 - 40) + 40),
        Min: lb - Math.floor(Math.random() * 20),
        Max: lb + Math.floor(Math.random() * 20),
        Nmax: Math.floor(Math.random() * (6 - 1) + 1),
        Nzeros: 0,
        Mode: lb + Math.floor(Math.random() * 4 - 2),
        Median: lb + Math.floor(Math.random() * 4 - 2),
        Variance: Math.floor(Math.random() * (30 - 10) + 10),
        Tendency: 0
      };
    } else if (rand < 0.75) {
      const lb = Math.floor(Math.random() * (165 - 150) + 150);
      const astv = Math.floor(Math.random() * (65 - 45) + 45);
      const altv = Math.floor(Math.random() * (45 - 20) + 20);
      const mstv = Number((Math.random() * (0.8 - 0.3) + 0.3).toFixed(1));
      p = {
        baseline: lb, ASTV: astv, DP: 0, Mean: lb, ALTV: altv, MSTV: mstv, AC: 0,
        fetalMovement: 0,
        uterineContractions: Number((Math.random() * (0.008 - 0.004) + 0.004).toFixed(3)),
        lightDecel: Number((Math.random() * (0.008 - 0.003) + 0.003).toFixed(3)),
        severeDecel: 0,
        MLTV: Number((Math.random() * (10 - 2) + 2).toFixed(1)),
        Width: Math.floor(Math.random() * (60 - 20) + 20),
        Min: lb - Math.floor(Math.random() * 30),
        Max: lb + Math.floor(Math.random() * 10),
        Nmax: Math.floor(Math.random() * 4),
        Nzeros: Math.floor(Math.random() * 2),
        Mode: lb - Math.floor(Math.random() * 10),
        Median: lb - Math.floor(Math.random() * 5),
        Variance: Math.floor(Math.random() * (50 - 20) + 20),
        Tendency: -1
      };
    } else {
      const lb = Math.floor(Math.random() * (110 - 80) + 80);
      const astv = Math.floor(Math.random() * (90 - 70) + 70);
      const dp = Number((Math.random() * (0.015 - 0.005) + 0.005).toFixed(3));
      const altv = Math.floor(Math.random() * (90 - 50) + 50);
      const mstv = Number((Math.random() * 0.5).toFixed(1));
      const sDecel = Number((Math.random() * (0.01 - 0.003) + 0.003).toFixed(3));
      p = {
        baseline: lb, ASTV: astv, DP: dp, Mean: lb - 10, ALTV: altv, MSTV: mstv, AC: 0,
        fetalMovement: 0,
        uterineContractions: Number((Math.random() * (0.012 - 0.006) + 0.006).toFixed(3)),
        lightDecel: 0,
        severeDecel: sDecel,
        MLTV: Number((Math.random() * 5).toFixed(1)),
        Width: Math.floor(Math.random() * (150 - 80) + 80),
        Min: lb - Math.floor(Math.random() * 60),
        Max: lb + Math.floor(Math.random() * 5),
        Nmax: 0,
        Nzeros: 0,
        Mode: lb - Math.floor(Math.random() * 20),
        Median: lb - Math.floor(Math.random() * 15),
        Variance: Math.floor(Math.random() * (100 - 50) + 50),
        Tendency: -1
      };
    }
    ...
    <div className="flex flex-col xl:flex-row gap-6 items-start pb-10">
      ...
      <div className="flex-1 w-full flex flex-col gap-6">
        ...
        <p className="text-sm text-gray-500 mb-6 leading-relaxed max-w-xl">
          Enter real-time CTG data below or select a preset, then click Run Clinical Assessment. The diagnostic model will calculate clinical risk probabilities based on the Hammacher Gradient Boosting model.
        </p>

        {/* Quick Clinical Presets */}
        <div className="mb-6 p-3.5 bg-blue-50/50 rounded-xl border border-blue-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <span className="text-xs font-bold text-blue-900 uppercase tracking-wider">Fast Presets:</span>
          <div className="flex flex-wrap gap-2 w-full sm:w-auto">
            <button
              onClick={() => applyPreset('normal')}
              className="flex-1 sm:flex-none px-3.5 py-1.5 rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-200 text-xs font-bold hover:bg-emerald-100 transition cursor-pointer"
            >
              Normal (Class 1)
            </button>
            <button
              onClick={() => applyPreset('suspect')}
              className="flex-1 sm:flex-none px-3.5 py-1.5 rounded-lg bg-amber-50 text-amber-700 border border-amber-200 text-xs font-bold hover:bg-amber-100 transition cursor-pointer"
            >
              Suspect (Class 2)
            </button>
            <button
              onClick={() => applyPreset('pathological')}
              className="flex-1 sm:flex-none px-3.5 py-1.5 rounded-lg bg-rose-50 text-rose-700 border border-rose-200 text-xs font-bold hover:bg-rose-100 transition cursor-pointer"
            >
        ...
        <div className="flex flex-col sm:flex-row gap-4 mt-8 pt-8 border-t border-gray-100">
          ...
          >
            {isLoading ? 'Running Analysis...' : 'Run Clinical Assessment'}
          </motion.button>
        </div>
      </motion.div>
