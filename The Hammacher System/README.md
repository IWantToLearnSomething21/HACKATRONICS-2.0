...
> **Looking for the Hackathon Judge Showcase & Feature Pitch?**  
> Please see **[HACKATHON_SHOWCASE.md](HACKATHON_SHOWCASE.md)** for the complete judge overview, system architecture diagram, live Web Audio Doppler demo, Twilio dispatch workflow, and instant browser demo links!
...
Interpretability is mandatory for clinical decision-support adoption:
...
```
.
├── README.md               # Hackathon strategy and architecture doc (Project understanding)
├── HACKATHON_SHOWCASE.md   # Judge Showcase: Full app features, architecture & pitch
...
## 6. Verified Benchmark Results (5-Fold CV & Isolated Test Set)
...
## 7. Target Milestones & Verified Deliverables
- [x] **Milestone 1**: Project initialization, directory structure, requirements manifest, and rubric-aligned strategy.
- [x] **Milestone 2**: Ingest dataset (`data/raw/CTG.xls` & `ucimlrepo(id=193)`), validate target distribution (1: Normal, 2: Suspect, 3: Pathological) and create physiological feature dictionary ([EXPLAINABILITY.md](EXPLAINABILITY.md)).
- [x] **Milestone 3**: Clean, engineer, scale, and partition data with zero-leakage stratified splitting ([src/preprocessing.py](src/preprocessing.py)).
- [x] **Milestone 4**: Train Family A (Regularized Logistic Regression) & Family B (Balanced Random Forest & Gradient Boosted Trees) with 5-Fold Stratified Cross-Validation ([src/train_benchmark.py](src/train_benchmark.py)).
- [x] **Milestone 5**: Multi-Class Macro F1 Evaluation, $3 \times 3$ Normalized Confusion Matrices, TreeSHAP Explainability, and Obstetric Guideline Alignment ([src/explainability.py](src/explainability.py) & [CLINICAL_GUIDELINE_ALIGNMENT.md](CLINICAL_GUIDELINE_ALIGNMENT.md)).
- [x] **Milestone 6**: Interactive Bedside Clinician Dashboard & Real-Time Triage Interface ([proto-01.html](proto-01.html) & [Aura/](Aura/src/App.tsx)).
