  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setPatients(prevPatients => {
        return prevPatients.map((patient, idx) => {
          // 1. Generate next physiological sample from CTG simulator
          const sample = globalSimulator.generateNextSample(patient.id, 2.0);
          
          // 2. Run Real-Time Gradient Boosting Inference asynchronously (round-robin when not in Demo Mode)
          const shouldRunML = activeTab !== 'features' && (patient.id === selectedPatientId || idx === (Math.floor(Date.now() / 1000) % prevPatients.length));
    ...
    }, 1000); // 1Hz snappy sampling update
